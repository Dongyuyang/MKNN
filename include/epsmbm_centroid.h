...
class MyQueryStrategy2 : public SpatialIndex::IQueryStrategy
{
	private:
		...
		double m_lowerbound;     // m_lowerbound = (1 + err) * adist(qm, Q) as lower bound
	...
	public:
		MyQueryStrategy2(const Region &M, const vector <Point> & Q, const uint32_t n,
			 const double lowerbound, const uint32_t f)
			: m_M(M), m_Q(Q), m_n(n), m_lowerbound(lowerbound), 
				best_dist(numeric_limits<double>::max()), m_f(f), 
				indexIO(0), leafIO(0) {} 
					...
					{  
						...
						if (amindist3 < best_dist) // amindist(pj, M) < best_dist
						{
							...
							{	
								...
								else { /* best_dist > (1+err)adist(q_m, Q) */ }
							}
